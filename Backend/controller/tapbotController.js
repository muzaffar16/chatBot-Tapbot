...
class TapBotController extends BaseController {
  ...
  SendMailToAdmin = async (req) => {
    const { order_id, mobile_number, email = 'NULL', message } = req.body;
    const attachmentPath = req.file ? req.file.location : null;
    ...
    (async () => {
      const info = await transporter.sendMail({
        ...
        subject: `tapshop Complain by ${mobile_number}`,
        text: `
               Order id : ${order_id}
               Mobile number : ${mobile_number}
               Email : ${email}
               Message : ${message}
              Attachement url : ${attachmentPath}
        `, // plain‑text body
      });
  ...
  HandleComplainData = async (req, res) => {
    ...
    await this.SendMailToAdmin(req)
